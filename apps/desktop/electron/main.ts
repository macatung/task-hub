import { app, BrowserWindow, ipcMain, Tray, Menu, nativeImage, screen, dialog, clipboard, shell, safeStorage, Notification, globalShortcut } from 'electron';
...
function createWindow() {
  ...
  win.on('maximize', () => safeSend(win, 'window-maximized-state', true));
  win.on('unmaximize', () => safeSend(win, 'window-maximized-state', false));
  ...
  ipcMain.handle('app-show-notification', (_event, { title, body }: { title: string; body: string }) => {
    if (Notification.isSupported()) {
      new Notification({ title: title || 'Task Hub', body: body || '' }).show();
      return true;
    }
    return false;
  });
