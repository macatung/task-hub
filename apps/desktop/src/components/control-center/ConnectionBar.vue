...
defineEmits<{ sync: []; connect: []; disconnect: []; settings: []; requirement: []; docs: []; openHub: []; close: [] }>();
</script>
...
<template>
  <header class="drag-region flex min-h-16 items-center justify-between border-b border-slate-200 bg-white px-5 text-slate-900">
    ...
    <div class="no-drag cc-toolbar flex items-center gap-2 text-xs"><span v-if="lastSynced" class="hidden text-slate-400 lg:inline">Updated {{ lastSynced }}</span><button class="cc-button" :disabled="syncing" @click="$emit('sync')">{{ syncing ? 'Syncing…' : 'Sync now' }}</button><button class="cc-button" @click="$emit('requirement')">New from requirement</button><button class="cc-button" @click="$emit('docs')">Scan repo docs</button><button class="cc-button" @click="$emit('settings')">Settings</button><button v-if="credential" class="cc-button" @click="$emit('openHub')">Open Hub</button><button class="cc-button" @click="credential ? $emit('disconnect') : $emit('connect')">{{ credential ? 'Disconnect' : 'Connect Hub' }}</button><button class="px-2 text-lg text-slate-400 hover:text-slate-800" aria-label="Close" @click="$emit('close')">×</button></div>
  </header>
</template>
