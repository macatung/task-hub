...
class TaskHubMcpController extends \App\Http\Controllers\Controller
{
    ...
    private function tools(): array
    {
        return [
            ...
            ['name' => 'get_task_history', 'description' => 'Read the full End-to-End transition and audit history for a task, including actor attribution.', 'inputSchema' => ['type' => 'object', 'properties' => ['task_id' => ['type' => 'integer']], 'required' => ['task_id']]],
            ...
            ['name' => 'get_agent_run', 'description' => 'Read an auditable AgentRun with persisted normalized events and technical logs.', 'inputSchema' => ['type' => 'object', 'properties' => ['run_id' => ['type' => 'integer']], 'required' => ['run_id']]],
            ['name' => 'get_workflow_run_status', 'description' => 'Read the normalized CAO workflow status and step mapping for an AgentRun.', 'inputSchema' => ['type' => 'object', 'properties' => ['run_id' => ['type' => 'integer']], 'required' => ['run_id']]],
            ['name' => 'record_agent_run_event', 'description' => 'Record an idempotent CAO workflow or supervisor event for an AgentRun.', 'inputSchema' => ['type' => 'object', 'properties' => ['run_id' => ['type' => 'integer'], 'event_id' => ['type' => 'string'], 'event_type' => ['type' => 'string'], 'status' => ['type' => 'string'], 'stage' => ['type' => 'string'], 'payload' => ['type' => 'object']], 'required' => ['run_id', 'event_id', 'event_type']]],
            ...
            ['name' => 'create_work_item', 'description' => 'Create a new Task/Epic/Story work item in Task Hub.', 'inputSchema' => ['type' => 'object', 'properties' => [
                ...
                'epic_id' => ['type' => 'integer', 'description' => 'Parent Epic ID for a Story or Task.'],
    ...
    private function workflowStatus(AgentRun $run): array
    {
        $workflow = (array) data_get($run->metadata, 'workflow', []);
        $steps = (array) ($workflow['steps'] ?? []);
        $completed = (array) ($workflow['completed_steps'] ?? []);
        return [
            'parentRunId' => $run->id,
            'caoRunId' => (string) ($workflow['cao_run_id'] ?? ''),
            'kind' => $workflow['kind'] ?? ($run->run_type === 'epic' ? 'epic' : 'task'),
            'state' => $workflow['state'] ?? $run->status,
            'currentStep' => $workflow['current_step'] ?? null,
            'totalSteps' => (int) ($workflow['total_steps'] ?? count($steps)),
            'completedSteps' => $completed,
            'failedStep' => $workflow['failed_step'] ?? null,
            'steps' => $steps,
        ];
    }
    ...
    private function callTool(array $params, Request $request, array $payload): array
    {
        ...
        $data = match ($name) {
            ...
            'get_task_history' => app(\App\Services\TaskHistoryService::class)->getTaskHistory($this->resolveTask($args['task_id'] ?? null)),
            ...
            'get_agent_run' => $runController->show(Request::create('/', 'GET'), $this->resolveAgentRun($args))->getData(true),
            'get_workflow_run_status' => $this->workflowStatus($this->resolveAgentRun($args)),
            'record_agent_run_event' => $runController->event(Request::create('/', 'POST', $args), $this->resolveAgentRun($args))->getData(true),
