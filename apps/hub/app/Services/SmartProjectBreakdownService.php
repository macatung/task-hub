            ...
            foreach (($sprint['tasks'] ?? []) as $task) {
                ...
                if ($issueType === 'epic') {
                    $epics[] = [
                        ...
                        'priority' => in_array($task['priority'] ?? 'high', ['urgent', 'high', 'medium', 'low'], true) ? ($task['priority'] ?? 'high') : 'high',
                        'category' => trim((string) ($task['category'] ?? 'backend')) ?: 'backend',
                        'story_points' => max(1, min(100, (int) ($task['story_points'] ?? 8))),
                        'status' => in_array($task['status'] ?? 'todo', ['todo', 'in_progress'], true) ? ($task['status'] ?? 'todo') : 'todo',
