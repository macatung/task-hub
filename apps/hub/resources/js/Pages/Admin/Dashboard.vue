  ...
  <AdminLayout title="CMS Dashboard">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
      <div>
        <h1 class="text-2xl sm:text-3xl font-display font-extrabold text-white">
          CMS Administration & Traffic Operations
        </h1>
        <p class="text-xs sm:text-sm text-slate-400 font-sans mt-0.5">
          Monitor real-time visitor traffic, manage portfolio projects, and triage inbound inquiries.
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <Link href="/admin/contacts" class="p-5 rounded-2xl glass-panel border border-white/10 hover:border-phantom-mint/40 transition-all">
        <div class="flex items-center justify-between text-slate-400 mb-2">
          <span class="text-xs font-mono uppercase tracking-wider">Inquiry Inbox</span>
      ...
      <Link href="/admin/projects" class="p-5 rounded-2xl glass-panel border border-white/10 hover:border-talisman-gold/40 transition-all">
        <div class="flex items-center justify-between text-slate-400 mb-2">
          <span class="text-xs font-mono uppercase tracking-wider">Active Projects</span>
          <Icons name="Sparkles" :size="16" class="text-talisman-gold" />
        </div>
        <div class="text-2xl sm:text-3xl font-display font-extrabold text-white">
          {{ stats.total_projects }}
        </div>
        <span class="text-[11px] font-mono text-talisman-gold mt-1 block">{{ stats.featured_projects }} featured projects →</span>
      </Link>
      ...
      <Link href="/admin/analytics" class="p-5 rounded-2xl glass-panel border border-white/10 hover:border-phantom-cyan/40 transition-all">
        <div class="flex items-center justify-between text-slate-400 mb-2">
          <span class="text-xs font-mono uppercase tracking-wider">Traffic Analytics</span>
      ...
      <div class="p-5 rounded-2xl glass-panel border border-white/10">
        <div class="flex items-center justify-between text-slate-400 mb-2">
          <span class="text-xs font-mono uppercase tracking-wider">Server Status</span>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      >
        ...
        <span class="font-display font-bold text-xs text-white">Skills & Arsenal</span>
      ...
      >
        <span class="text-xl">⏳</span>
        <span class="font-display font-bold text-xs text-white">Career Chronicles</span>
        <span class="text-[10px] font-mono text-slate-400">Milestone Timeline</span>
      </Link>
      ...
      >
        <span class="text-xl">📝</span>
        <span class="font-display font-bold text-xs text-white">Articles & Blog</span>
      ...
      >
        ...
        <span class="font-display font-bold text-xs text-white">Settings & Profile</span>
    ...
    <div class="p-6 rounded-2xl glass-panel border border-white/10 text-left">
      <div class="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
        <div>
          <h2 class="text-lg font-display font-bold text-white">Recent Inbound Inquiries</h2>
          <p class="text-xs text-slate-400">Latest project proposals and messages submitted via the inquiry altar.</p>
        </div>
        <Link
          href="/admin/contacts"
      ...
      <div v-if="recent_contacts.length === 0" class="p-8 text-center text-slate-500 font-mono text-xs">
        No inquiries received yet. Submit a test proposal via the inquiry section on the homepage!
      </div>
        ...
        <table class="w-full text-left text-xs font-sans">
          <thead class="bg-midnight-900/60 border-b border-white/5 text-slate-400 font-mono text-[11px] uppercase tracking-wider">
            <tr>
              ...
              <th class="p-3">Quest Type</th>
              <th class="p-3">Offering</th>
