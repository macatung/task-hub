...
<template>
  <AdminLayout title="Article Management">
    <Head title="Articles & Blog — Admin CMS" />
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
      <div>
        <h1 class="text-2xl sm:text-3xl font-display font-extrabold text-white">
          Engineering Articles & Blog CMS
        </h1>
        <p class="text-xs sm:text-sm text-slate-400 font-sans mt-0.5">
          Author and publish technical engineering guides, architecture case studies, and markdown posts.
        </p>
      </div>
    ...
    <div class="p-6 rounded-2xl glass-panel border border-white/10 text-left">
      <div v-if="articles.length === 0" class="p-12 text-center text-slate-500 font-mono text-xs">
        No articles created yet. Click "Write New Article" to compose your first post!
      </div>
      ...
      <div class="w-full max-w-3xl bg-midnight-900 border border-white/10 rounded-3xl p-6 sm:p-8 text-left shadow-2xl space-y-5">
        ...
        <form class="space-y-4" @submit.prevent="submitArticle">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-mono text-slate-300 uppercase mb-1">Article Title *</label>
              <input
                v-model="form.title"
                type="text"
                required
                ...
                placeholder="e.g. Optimizing High-Throughput Payment Gateways with Redis"
              />
            </div>
            <div>
              <label class="block text-xs font-mono text-slate-300 uppercase mb-1">URL Slug</label>
              <input
                v-model="form.slug"
                type="text"
                ...
                placeholder="optimizing-payment-gateways-redis"
              />
            </div>
          </div>

          <div>
            <label class="block text-xs font-mono text-slate-300 uppercase mb-1">Short Excerpt</label>
            <input
              v-model="form.excerpt"
              type="text"
              class="w-full px-3.5 py-2.5 rounded-xl bg-midnight-950 border border-white/10 text-white text-sm focus:border-phantom-mint focus:outline-none"
              placeholder="Summary of key technical points in 1-2 sentences..."
            />
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-mono text-slate-300 uppercase mb-1">Tags (Comma-separated)</label>
              <input
            ...
            <div>
              <label class="block text-xs font-mono text-slate-300 uppercase mb-1">Reading Time (Minutes)</label>
              <input
          ...
          <div>
            <label class="block text-xs font-mono text-slate-300 uppercase mb-1">Article Body (Markdown Format) *</label>
            <textarea
              ...
              placeholder="## Architecture Overview&#10;&#10;Full article body supporting **Markdown**..."
            />
          </div>
          ...
          <div class="flex items-center gap-2 pt-1">
            ...
            <label for="publish-checkbox" class="text-xs font-mono text-slate-300">
              Publish article immediately (Publicly Available)
            </label>
          </div>
